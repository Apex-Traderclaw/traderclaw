...
// =============================================================================
// Alpha state singletons (gateway-process-scoped)
//
// Why these live on globalThis and not inside register():
//
// Every plugin re-register (≈once per agent turn / Telegram interaction /
// config reload) creates a NEW AlphaBuffer and a NEW AlphaStreamManager.
// Without these singletons, the previous register's buffer and
// `messageCount` were thrown away, so users got "0 signals received" from
// the live tool seconds after they messaged the agent — even when the WS
// was actively delivering signals.
//
// The lifecycle's dispose() still tears down the WebSocket and timers from
// the prior register (no orphan connections), but the BUFFER CONTENTS and
// the LIFETIME COUNTER survive on globalThis. New managers read+write the
// same instances, so `solana_alpha_signals` returns "messages since this
// gateway process started" rather than "messages since the last
// re-register".
//
// Gateway restart still resets these (process exits → globalThis dies).
// Cross-restart persistence would require a sidecar file; not done here.
// =============================================================================
const SOLANA_TRADER_ALPHA_BUFFER_SINGLETON_KEY = Symbol.for(
...
function getOrCreateAlphaBuffer(): AlphaBuffer {
  const existing = __solanaTraderAlphaSingletonHolder[
    SOLANA_TRADER_ALPHA_BUFFER_SINGLETON_KEY
  ] as AlphaBuffer | undefined;
...
function getOrCreateAlphaLifetimeState(): AlphaLifetimeState {
  const existing = __solanaTraderAlphaSingletonHolder[
    SOLANA_TRADER_ALPHA_LIFETIME_SINGLETON_KEY
  ] as AlphaLifetimeState | undefined;
  if (existing) return existing;
  const fresh: AlphaLifetimeState = {
...
interface PluginConfig {
   ...
   * Legacy `forwardTelegramChatId` in JSON is still read. Use `telegramBotToken` in plugin config to resolve @usernames.
   */
  forwardTelegramRecipient?: string;
  /** Telegram bot token (@BotFather); plugin config only — required to resolve @username to chat id. */
  ...
  const forwardTelegramRecipient =
    typeof obj.forwardTelegramRecipient === "string"
      ...
      : typeof obj.forwardTelegramChatId === "string"
        ...
        : undefined;
  const telegramBotToken = typeof obj.telegramBotToken === "string" ? obj.telegramBotToken : undefined;
...
const solanaTraderPlugin = {
  ...
  register(api: OpenClawPluginApi): void {
    ...
    const patch = async (apiPath: string, body: Record<string, unknown>) => {
      const token = await sessionManager.getAccessToken();
      return orchestratorRequest({
        baseUrl: orchestratorUrl,
        method: "PATCH",
    ...
    const resolveStrategyField = (state: Record<string, unknown>, field: string): unknown => {
      return getNestedValue(state, `strategy.${field}`) ?? state[field];
    };

    const renderKeyValueSection = (obj: unknown, indent = ""): string[] => {
    ...
    const generateMemoryMd = (aid: string, stateObj: unknown): string => {
      ...
      const identityFields: [string, string][] = [
        ["tier", "Tier"], ["walletId", "Wallet"], ["mode", "Mode"],
        ["strategyVersion", "Strategy Version"], ["regime", "Regime"],
        ["maxPositions", "Max Positions"], ["minBuy", "Min Buy"], ["minTp", "Min TP"],
      ];
      ...
      if (state.defenseMode !== undefined) lines.push(`## Defense Mode`, "", `- Active: ${state.defenseMode}`, "");
      if (state.killSwitchActive !== undefined) lines.push(`## Kill Switch`, "", `- Active: ${state.killSwitchActive}`, "");

      if (identityLines.length > 0 || state.defenseMode !== undefined || state.killSwitchActive !== undefined) {
        lines.push("---", "");
      }

      const strategyHasContent = ["featureWeights", "positionSizing", "stopLoss", "takeProfit", "entryFilter", "deadMoney"]
        .some((f) => resolveStrategyField(state, f) !== undefined);
      ...
      if (state.permanentLearnings && Array.isArray(state.permanentLearnings)) {
        ...
        state.permanentLearnings.slice(0, 30).forEach((learning: unknown, i: number) => {
      ...
      if (capitalStatus && typeof capitalStatus === "object") {
        const cs = capitalStatus as Record<string, unknown>;
        lines.push("## Capital Status Tracker", "");
        if (cs.lastUpdated) lines.push(`Last Updated: ${cs.lastUpdated}`);
        const csFields: [string, string][] = [
          ["walletBalance", "Wallet Balance"], ["realizedPnlAllTime", "Realized P&L (All-Time)"],
          ["openPositions", "Open Positions"], ["dailyNotionalUsed", "Daily Notional Used"],
          ["dailyRealizedLoss", "Daily Realized Loss"], ["killSwitch", "Kill Switch"],
        ];
        for (const [key, label] of csFields) {
          if (cs[key] !== undefined) lines.push(`- ${label}: ${formatStateValue(cs[key])}`);
        }
        const renderedCsKeys = new Set(["lastUpdated", ...csFields.map(([k]) => k)]);
        for (const [k, v] of Object.entries(cs)) {
          if (!renderedCsKeys.has(k) && v !== undefined) lines.push(`- ${k}: ${formatStateValue(v)}`);
        }
        lines.push("", "---", "");
      }
      ...
      if (nextCycle && typeof nextCycle === "object") {
        const nc = nextCycle as Record<string, unknown>;
        lines.push("## Next Trading Cycle", "");
        const ncFields: [string, string][] = [
          ["scan", "Scan"], ["candidatesInLab", "Candidates in Lab"],
          ["strategy", "Strategy"], ["focus", "Focus"], ["action", "Action"],
        ];
        for (const [key, label] of ncFields) {
          if (nc[key] !== undefined) lines.push(`- ${label}: ${formatStateValue(nc[key])}`);
        }
        const renderedNcKeys = new Set(ncFields.map(([k]) => k));
        for (const [k, v] of Object.entries(nc)) {
          if (!renderedNcKeys.has(k) && v !== undefined) lines.push(`- ${k}: ${formatStateValue(v)}`);
        }
        lines.push("");
      }

      const structuredKeys = new Set([
        "tier", "walletId", "mode", "strategyVersion", "regime", "maxPositions", "maxPositionSizeSol",
        "defenseMode", "killSwitchActive", "watchlist", "permanentLearnings", "regimeCanary", "preferences",
        "minBuy", "minTp", "tpStructure",
        "strategy", "featureWeights", "positionSizing", "stopLoss", "takeProfit", "entryFilter", "deadMoney",
        "capitalStatus", "nextCycle",
      ]);
    ...
    const writeMemoryMd = (aid: string, stateObj: unknown): boolean => {
      try {
    ...
    api.registerTool({
      ...
      parameters: Type.Object({
        ...
        agentId: Type.Optional(
          Type.String({
            description:
              "Optional agent id for gateway-forwarded notices (e.g. main). If omitted, the plugin config `agentId` is sent when set so default-risk messages reach the right agent.",
          }),
        ),
      }),
      execute: wrapExecute("solana_trade_execute", async (_id, params) => {
        ...
        const execAgentId =
          typeof params.agentId === "string" && params.agentId.trim().length > 0
            ? params.agentId.trim()
            : config.agentId && String(config.agentId).trim().length > 0
              ? String(config.agentId).trim()
              : undefined;
    ...
    api.registerTool({
      ...
      description:
        ...
        "**Exit plan (source of truth):** use each position's `tpLevelsDetailed`, `slLevels`, `trailingStopPct`, `trailingStopLevels`, and `deadlockState.exits` — never guess exits from mode tables or memory.",
      parameters: Type.Object({
    ...
    api.registerTool({
      ...
      description:
        "Read per-wallet default exit parameters used when a buy is executed **without** any TP/SL/trailing fields. Returns either the user's saved defaults or the platform system default (`source`: user | system).",
      parameters: Type.Object({}),
      execute: wrapExecute("risk_management_get_default", async () =>
        get(`/api/wallet/risk-defaults?walletId=${walletId}`),
      ),
    });
    ...
    api.registerTool({
      ...
      execute: wrapExecute("risk_management_set_default", async (_id, params) =>
        put("/api/wallet/risk-defaults", {
          walletId,
    ...
    api.registerTool({
      ...
      description:
        "Update **percentages and amounts only** on an open position's live exit plan (CaptureSell + stored position metadata). Cannot add or remove levels — array lengths must match the existing subscription. Use after entry when refining TP/SL/trailing numerics.",
      parameters: Type.Object({
    ...
    api.registerTool({
      ...
      description:
        "Read your TraderClaw referral profile: your current referral code (null if not set), access window, tier, and earnings metadata. " +
        "Call this when the user asks about their referral code, when ACCESS_LIMIT_REACHED is received, or to check whether a code has already been created. " +
        "This endpoint remains accessible even after the runtime access window has expired — use it to help the user set up a referral code so they can regain access by referring others. " +
        "If referralCode is null, prompt the user to choose a custom 4–16 alphanumeric code and call solana_referral_set_code with their input.",
      parameters: Type.Object({}),
    ...
    api.registerTool({
      ...
      description:
        "Set or update your custom TraderClaw referral code. The code must be 4–16 alphanumeric characters; it is stored normalized to UPPERCASE. " +
        "IMPORTANT: You must ask the user to provide the code — never invent or guess one. Suggest that the code be memorable (e.g. their username, brand, or handle). " +
        "Once set, anyone who signs up using this code grants you +8 hours of access per active referral. " +
        "This endpoint remains accessible even after the runtime access window has expired, so the user can always create or update their referral code. " +
        "After setting the code, confirm the saved value from the response and share it with the user so they can start referring others. " +
        "Expected errors (check ok=false + errorCode): " +
        "REFERRAL_CODE_TAKEN — that exact code is already used by another account; ask the user to choose a different one. " +
        "VALIDATION_ERROR — code does not meet the 4–16 alphanumeric rule; tell the user which constraint was violated. " +
        "AUTH_ERROR — session expired; user must re-authenticate before setting the code.",
      parameters: Type.Object({
        referralCode: Type.String({
          description:
            "Custom referral code chosen by the user. Must be 4–16 alphanumeric characters (letters and digits only). Will be stored as UPPERCASE.",
          minLength: 4,
          maxLength: 16,
        }),
      }),
      execute: wrapExecute("solana_referral_set_code", async (_id, params) =>
        put("/api/referral/code", { referralCode: String(params.referralCode).trim().toUpperCase() }),
      ),
    });
    ...
    api.registerTool({
      ...
      parameters: Type.Object({
        ...
        forwardTelegramRecipient: Type.Optional(
          Type.String({
            description:
              "Telegram @username or numeric chat id. Resolved to chat id via Bot API when not numeric (requires telegramBotToken in plugin config).",
          }),
        ),
    ...
    // Re-use the AlphaBuffer + lifetime counter from the previous register
    // (or create them on first boot). This makes "how many alpha signals
    // have we received?" return the gateway-process total rather than only
    // counting messages since the latest agent turn / Telegram interaction
    // (which re-registers the plugin and would otherwise zero everything).
