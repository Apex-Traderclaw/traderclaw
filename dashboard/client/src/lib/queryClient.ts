...
/** Turn API JSON (or plain text) into a short, wrap-friendly message for toasts. */
function formatApiErrorText(status: number, bodyText: string): string {
  ...
  try {
    ...
    if (code === 'ACCESS_RUNTIME_PAUSED') {
      const head =
        msg ||
        'Agent runtime is paused. This is not the same as trial time running out—resume from the Dashboard when you want the agent back.';
      return `${head}\n(${code})`;
    }

    if (code === 'ENTITLEMENT_INSUFFICIENT_FUNDS') {
      const need = j.requiredSol;
      const have = j.balanceSol;
      const head = msg || 'Insufficient balance for this purchase';
      const detail =
        (typeof need === 'number' || typeof need === 'string') && (typeof have === 'number' || typeof have === 'string')
          ? `Requires ${need} SOL · Your balance: ${have} SOL`
          : '';
      return detail ? `${head}\n${detail}` : head;
    }
  ...
  const body = trimmed.length > 400 ? `${trimmed.slice(0, 400)}\u2026` : trimmed;
  return `Request failed (HTTP ${status})\n${body}`;
}
...
async function throwIfResNotOk(res: Response) {
  if (!res.ok) {
    ...
    throw new Error(formatApiErrorText(res.status, text));
  }
}
...
/** End dashboard session and clear stored API key (use for sign-out / switching accounts). */
