...
var SessionManager = class {
  ...
  async initialize() {
    ...
    if (challenge.walletProofRequired && challenge.challenge) {
      ...
      if (!walletPrivateKey) {
        throw new Error(
          "Wallet proof required but no walletPrivateKey configured. This account already has a wallet \u2014 set TRADERCLAW_WALLET_PRIVATE_KEY in the host runtime environment and restart the gateway. The key is used only for local signing and is never sent to the orchestrator. Do not store private keys in openclaw.json."
        );
      }
