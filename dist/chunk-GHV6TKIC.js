...
var AlphaStreamManager = class {
  ...
  async connect() {
    ...
    const url = `${this.config.wsUrl}?accessToken=${token}`;
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on("open", () => {
        ...
        this.log("info", "WebSocket connected, sending alpha_stream_subscribe");
  ...
  handleMessage(msg) {
    switch (msg.type) {
      ...
      case "error": {
        ...
        if (code === "WS_AUTH_REQUIRED" || code === "WS_AUTH_INVALID" || code === "WS_SESSION_INVALID") {
          this.log("warn", "Auth error \u2014 will reconnect with fresh token");
